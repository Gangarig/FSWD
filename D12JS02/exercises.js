...
function average_Grade(Math,Physics,English) {
    return "Average: "+(Math+Physics+English)/3 +br+ "Sum: "+(Math+Physics+English)+br+hr;
}
...
function atmnotecalculator (givenamount) {
    if (givenamount >= 10) {
        ...
        let rest = givenamount - 100*euro100-50*euro50-20*euro20-10*euro10;   
        if (rest>0) {
            document.write(`\n${rest} is not possible ${br}`)
        }  else {}
        return `\n 100 notes --- ${euro100} ${br} 50 notes ---${euro50}${br} \n 20 notes ---${euro20}${br} \n 10 notes ---${euro10}${br} ` 
    } else 
        {
        return 'Wrong amount'
        }
...
document.write(atmnotecalculator(647));
